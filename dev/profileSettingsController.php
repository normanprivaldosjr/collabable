...
if($stmt->execute()){
    $stmt->store_result();
    if($stmt->num_rows > 0){
        $stmt->bind_result($UID, $empName, $empUserName, $empPassword, $empEmailAdd, $empContactNum, $empRole);
        $stmt->fetch();
    }else{
...
if(isset($_POST['submit'])){
    ...
    $sql = "UPDATE users SET employeeName = ? , employeeUserName = ?, employeePassWord = ?, employeeEmailAddress = ?, employeeContactNum = ? WHERE employeeId = $userid";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("sssss" , $name, $userName, $password, $emailAdd, $contactNum);
    if($stmt->execute()){
